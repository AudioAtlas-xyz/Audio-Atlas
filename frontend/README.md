...
OAuth login redirects use the public runtime variable `NUXT_PUBLIC_BACKEND_BASE_URL`.
...
On a real deployment target, start the built Nitro server with your platform's normal Nuxt/Nitro startup command and make sure both variables are present in the environment.
...
If you need to override the browser API base for a special deployment, set `NUXT_PUBLIC_API_BASE`.
